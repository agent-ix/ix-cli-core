...
A plugin's schema is declared through the `ixSchema` named export ([FR-014](./FR-014-ixschema-plugin-convention.md)). `@agent-ix/ix-cli-core` SHALL expose a registration surface accepting the following shape:
...
| FR-004-AC-6 | A `secrets` entry with `envVar: "IX_FOO"` causes `SecretsService.get('<id>.foo')` to honor `IX_FOO` ahead of any persisted backend (per [FR-005](./FR-005-secrets-service-api.md) resolution order).                                                                                                                                                                                                                                                                                                                                | Test         |
...
- **Upstream**: [StR-001](../stakeholder/StR-001-pluggable-config-contract.md) (implements), [FR-001](./FR-001-config-service-api.md) (requires), [FR-005](./FR-005-secrets-service-api.md) (requires)
...
| `SecretDeclaration` (shape)   | `{ name: string; description: string; required?: boolean; envVar?: string }`                                   | —                                                                                                                                                                                                       | Registered globally as `<pluginId>.<name>`. `envVar`, when set, takes precedence over backend values in `SecretsService.get` ([FR-005-AC-1](./FR-005-secrets-service-api.md)).                                                                                                                                                                                                |
