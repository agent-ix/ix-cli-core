...
**Schema enforcement.** Schemas SHALL be Zod `.strict()` (per [FR-004](./FR-004-plugin-schema-registration.md)); writes that introduce unknown keys SHALL throw `ConfigSchemaError`. Reads of an existing-but-invalid file SHALL throw `ConfigParseError`; the loader callsite ([FR-002](./FR-002-per-plugin-file-isolation.md)) decides whether to fall back to defaults.
...
- _Symlinked target_: refused on read per [NFR-002-AC-4](../non-functional/NFR-002-sensitive-file-permissions.md); on write, the symlink is detected before any operation and refused with `ConfigSymlinkRefusedError` naming both the symlink path and its target.
...
- **Upstream**: [StR-001](../stakeholder/StR-001-pluggable-config-contract.md) (implements), [FR-002](./FR-002-per-plugin-file-isolation.md) (requires), [FR-004](./FR-004-plugin-schema-registration.md) (requires)
...
| `PluginConfig.get`        | `() => T`                                                                                                                          | `T`               | Reads env → file → schema defaults ([FR-003](./FR-003-layered-resolution.md)). Parse/validation failures are recorded as incidents and defaults are returned; never throws ([FR-002-AC-1](./FR-002-per-plugin-file-isolation.md)).                                                                                                                                                                                                                                             |
| `PluginConfig.set`        | `(partial: Partial<T>) => void`                                                                                                    | `void`            | Deep-merges `partial` into current on-disk content (arrays replace wholesale), validates against the strict schema, then atomically writes via temp+rename with mode `0o600`. Throws `ConfigSchemaError` on validation failure, `ConfigWriteError` on `EACCES`/`EROFS`/`ENOSPC`, `ConfigSymlinkRefusedError` if the target is a symlink. Serialized under the per-file advisory lock ([FR-002-AC-4](./FR-002-per-plugin-file-isolation.md)). |
